...
function init() {
    ...
    arrOfSquare = [];
    arrOfCoordinates = [];
    for (let i = 0; i < maxSquares; i++) {getRandomElement
...
function handleTimerEnd() {
    ...
    if (score >= nextLevelScore) {
        ...
        level = 1;
        let message = 'Время вышло!ffffffffffffffff\nХотите попробовать ещё раз?';
        alert(message);
        let result = confirm(message);
        if (result) {
            location.reload();
...
let activeElement = null; // Создаем переменную для хранения активного элемента

document.onmousedown = function (e) {
    const targetElement = e.target.parentElement;
    if (targetElement.index === undefined) return;

    activeElement = targetElement; // Сохраняем элемент в переменной активного элемента
    activeElement.classList.add('active'); // Делаем элемент активным
    document.onmousemove = function (e) {
        activeElement.style.left = e.pageX - parseInt(activeElement.style.width) / 2 + 'px';
        activeElement.style.top = e.pageY - parseInt(activeElement.style.height) / 2 + 'px';
    };
    ...
    document.onmouseup = function (e) {
        ...
        const field_for_answer = document.querySelector('.field_for_answer');
        const targetX = activeElement.getBoundingClientRect().left;
        const targetY = activeElement.getBoundingClientRect().top;
        const pageX = field_for_answer.getBoundingClientRect().left;
        const pageY = field_for_answer.getBoundingClientRect().top;
        if (Math.abs(targetX - pageX) < 25 && Math.abs(targetY - pageY) < 25) {
            if (activeElement.index === indexWinSquare) {
                winAnimation();
            } else {
                lose();
            }
...
document.onkeydown = function (e) { // поворот по нажатию стелочек на клавиатуре на первом уровне
    if (!activeElement) return; // Если нет активного элемента, ничего не делаем
    if (level !== 1) return;
    if (e.key === 'ArrowLeft') {
        activeElement.angle -= 90;
        activeElement.style.transform = `rotate(${activeElement.angle}deg)`;
        check();
    }
...
document.ondblclick = function (e) {  // поворот при дабл клике на втором уровне
    if (!activeElement) return; // Если нет активного элемента, ничего не делаем
    if (level !== 2) return;
    activeElement.angle += 90; // Поворачиваем элемент на 90 градусов
    activeElement.style.transform = `rotate(${activeElement.angle}deg)`;
    check();
};